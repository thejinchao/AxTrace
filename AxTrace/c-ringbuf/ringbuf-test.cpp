...
int _ftruncate(FILE* file, unsigned int len)
{
	fseek(file, len, SEEK_SET);
	int fd = _fileno(file);
	HANDLE hfile = (HANDLE)_get_osfhandle(fd);
	return SetEndOfFile(hfile)==TRUE ? 0 : 1;
}

FILE* rdfd = 0;
FILE* wrfd = 0;
char rd_template[] = "./tmpXXXXXXringbuf.tmp";
char wr_template[] = "./tmpXXXXXXringbuf-wr.tmp";
...
{
	double ddd = 1.1234567890123456789012345678901;
	float fff = (float)ddd;
	char temp[512];
	_gcvt_s(temp, 512, ddd, 16);
	//_snprintf(temp, 512, "%f|%.16f|%G|%.16f", ddd, fff, ddd, ddd);

	return 0;

	wchar_t* wszChar = L"hello½ù³¬world";
	const unsigned char* pszChar = (unsigned char*)"\x68\x65\x6C\x6C\x6F\xE9\x9D\xB3\xE8\xB6\x85\x77\x6F\x72\x6C\x64";

	unsigned char utf8[1024]={0};
	unsigned char* pUtf8 = utf8;

	wchar_t wszBuf[256]={0};
	unsigned short* p_wszBuf = (unsigned short*)wszBuf;

	ConvertUTF8toUTF16(&pszChar, pszChar+strlen((const char*)pszChar), &p_wszBuf, p_wszBuf+8, strictConversion);

	//ConvertUTF16toUTF8((const unsigned short**)&wszChar, (const unsigned short*)wszChar+24, &pUtf8, pUtf8+1024, strictConversion);
	return 0;
}
...
main_bak(int argc, char **argv)
{
    ...
    const uint8_t *rb1_base = (const uint8_t *)ringbuf_head(rb1);
    ...
    /* Different sizes */
    rb1 = ringbuf_new(24);
    rb1_base = (const uint8_t *)ringbuf_head(rb1);
    ...
    uint8_t *buf = (uint8_t *)malloc(RINGBUF_SIZE * 2);
    ...
    assert(!ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp((const char *) buf, (const char*)ringbuf_tail(rb1), 7) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf, RINGBUF_SIZE - 1) == 0);
    END_TEST(test_num);
    ...
    assert(!ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp((const char *) buf, (const char*)ringbuf_tail(rb1), 7 + 15) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp((const char *) buf, (const char*)ringbuf_tail(rb1), RINGBUF_SIZE - 1) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_head(rb1) == rb1_base);
    assert(ringbuf_tail(rb1) == rb1_base + 1);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + RINGBUF_SIZE, RINGBUF_SIZE - 1) == 0);
    END_TEST(test_num);
    ...
    assert(!ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp(test_pattern, (const char*)ringbuf_tail(rb1), strlen(test_pattern)) == 0);
    assert(*(char *)ringbuf_head(rb1) == '\1');
    END_TEST(test_num);
    ...
    assert(!ringbuf_is_full(rb1));
    assert(!ringbuf_is_empty(rb1));
    assert(strncmp((const char *) buf, (const char*)ringbuf_tail(rb1), 2 * strlen(test_pattern) - 1) == 0);
    assert(*(char *)ringbuf_head(rb1) == '\1');
    END_TEST(test_num);
    ...
    /* tail should have bumped forward by 1 byte */
    assert(ringbuf_tail(rb1) == rb1_base + 1);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + 1, RINGBUF_SIZE - 1) == 0);
    END_TEST(test_num);
    ...
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + 2, RINGBUF_SIZE - 2) == 0);
    assert(strncmp((const char *) rb1_base, (const char *) buf + RINGBUF_SIZE, 1) == 0);
    END_TEST(test_num);

    rdfd = fopen(rd_template, "wb");
    assert(rdfd != 0);
    assert(fwrite(buf, RINGBUF_SIZE, 2, rdfd) == 2);
	fclose(rdfd);
	rdfd = fopen(rd_template, "rb");
    assert(rdfd != 0);
    
    /* ringbuf_read with zero count */
    START_NEW_TEST(test_num);
    ringbuf_reset(rb1);
    assert(fseek(rdfd, 0, SEEK_SET) == 0);
    ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
    ringbuf_reset(rb1);
    ...
    assert(ringbuf_tail(rb1) == ringbuf_head(rb1));
    assert(*(char *)ringbuf_head(rb1) == '\1');
    assert(fseek(rdfd, 0, SEEK_CUR) == 0);
    END_TEST(test_num);
    ...
    uint8_t *dst = (uint8_t*)malloc(RINGBUF_SIZE * 2);
    ...
    uint8_t *buf2 = (uint8_t*)malloc(RINGBUF_SIZE * 2);
    ...
    ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
    ringbuf_reset(rb1);
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    ...
    assert(ringbuf_head(rb1) == (uint8_t *) ringbuf_tail(rb1) + (strlen(test_pattern2) - 3));
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    assert(fread(dst, 1, 4, wrfd) == 3);
    assert(fread(dst + 3, 1, 1, wrfd) == 0);
    assert(strncmp((const char *) dst, test_pattern2, 3) == 0);
    assert(strncmp((const char *) dst + 3, (const char *) buf + 3, RINGBUF_SIZE * 2 - 3) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_head(rb1) == rb1_base + RINGBUF_SIZE - 1);
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    assert(fread(dst, RINGBUF_SIZE, 1, wrfd) == RINGBUF_SIZE - 1);
    assert(fread(dst + RINGBUF_SIZE - 1, 1, 1, wrfd) == 0);
    assert(strncmp((const char *) dst, (const char *) buf2, RINGBUF_SIZE - 1) == 0);
    assert(strncmp((const char *) dst + RINGBUF_SIZE - 1, (const char *) buf + RINGBUF_SIZE - 1, RINGBUF_SIZE + 1) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_tail(rb1) == rb1_base + 13);
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    assert(fread(dst, 14, 1, wrfd) == 13);
    assert(fread(dst + 13, 1, 1, wrfd) == 0);
    assert(strncmp((const char *) dst, (const char *) buf2, 13) == 0);
    assert(strncmp((const char *) dst + 13, (const char *) buf + 13, RINGBUF_SIZE * 2 - 13) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_tail(rb1) == rb1_base + RINGBUF_SIZE - 1);
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    assert(fread(dst, RINGBUF_SIZE - 1, 1, wrfd) == RINGBUF_SIZE - 1);
    assert(fread(dst + RINGBUF_SIZE - 1, 1, 1, wrfd) == 0);
    assert(strncmp((const char *) dst, (const char *) buf2, RINGBUF_SIZE - 1) == 0);
    assert(strncmp((const char *) dst + RINGBUF_SIZE - 1, (const char *) buf + RINGBUF_SIZE - 1, RINGBUF_SIZE * 2 - (RINGBUF_SIZE -1)) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_head(rb1) == rb1_base + 1);
    assert(fseek(wrfd, 0, SEEK_SET) == 0);
    assert(fread(dst, RINGBUF_SIZE, 1, wrfd) == RINGBUF_SIZE);
    assert(fread(dst, 1, 1, wrfd) == 0);
    ...
    const uint8_t *rb2_base = (const uint8_t *)ringbuf_head(rb2);
    ...
    assert(ringbuf_head(rb1) == rb1_base + 2);
    assert(ringbuf_head(rb2) == rb2_base + 2);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf2, 2) == 0);
    END_TEST(test_num);
    ...
    assert(ringbuf_head(rb2) == rb2_base + 2);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf, 3) == 0);
    ...
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + 1, RINGBUF_SIZE - 2) == 0);
    ...
    assert(ringbuf_head(rb1) == rb1_base + 1);
    assert(ringbuf_head(rb2) == rb2_base + 2);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + 2, RINGBUF_SIZE - 3) == 0);
    ...
    assert(ringbuf_head(rb2) == rb2_base + 1);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf2 + RINGBUF_SIZE - 3, 3) == 0);
    assert(*(char *)ringbuf_head(rb1) == '\1');
    END_TEST(test_num);
    ...
    assert(ringbuf_head(rb2) == rb2_base + 1);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf2 + RINGBUF_SIZE - 3, 4) == 0);
    assert(*(char *)ringbuf_head(rb1) == '\1');
    END_TEST(test_num);
    ...
    /* one byte from buf */
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + RINGBUF_SIZE - 4, 1) == 0);
    ...
    assert(ringbuf_head(rb1) == rb1_base + 1);
    assert(ringbuf_head(rb2) == rb2_base + 2);
    assert(strncmp((const char*)ringbuf_tail(rb1), (const char *) buf + 2, RINGBUF_SIZE - 1 - 2) == 0);
    ...
    const uint8_t *rb3_base = (const uint8_t *)ringbuf_head(rb3);
    ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
    ringbuf_reset(rb1);
	...
	//-----MY---------
	ringbuf_t pRingBuf = ringbuf_new(6);
	ringbuf_memcpy_into(pRingBuf, "Hello", 5);
	size_t c = ringbuf_bytes_free(pRingBuf);

	size_t s = ringbuf_bytes_used(pRingBuf);
	ringbuf_memcpy_into(pRingBuf, "World", 5);
	s = ringbuf_bytes_used(pRingBuf);

	ringbuf_t tmpRingBuf = ringbuf_new(ringbuf_bytes_used(pRingBuf));
	ringbuf_copy(tmpRingBuf, pRingBuf, ringbuf_bytes_used(pRingBuf));

	char temp[16];
	ringbuf_memcpy_from(temp, pRingBuf, 5);
	s = ringbuf_bytes_used(tmpRingBuf);

	ringbuf_memcpy_from(temp, pRingBuf, 5);
	s = ringbuf_bytes_used(tmpRingBuf);

	ringbuf_free(&tmpRingBuf);
	ringbuf_free(&pRingBuf);
