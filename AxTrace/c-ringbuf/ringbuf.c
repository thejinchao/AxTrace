...
ringbuf_findchr(const struct ringbuf *rb, int c, size_t offset)
{
    ...
    assert(bufend > start);
    n = (size_t)MIN((size_t)(bufend - start), bytes_used - offset);
    found = memchr(start, c, n);
    if (found)
...
ringbuf_memset(ringbuf_t dst, int c, size_t len)
{
    ...
    while (nwritten != count) {
        ...
        n = (size_t)MIN((size_t)(bufend - dst->head), count - nwritten);
        memset(dst->head, c, n);
        dst->head += n;
        nwritten += n;
...
ringbuf_memcpy_into(ringbuf_t dst, const void *src, size_t count)
{
    ...
    while (nread != count) {
        /* don't copy beyond the end of the buffer */
        assert(bufend > dst->head);
        n = (size_t)MIN((size_t)(bufend - dst->head), count - nread);
...
ssize_t
ringbuf_read(FILE* fd, ringbuf_t rb, size_t count)
{
	size_t tmp = ftell(fd);
    const uint8_t *bufend = ringbuf_end(rb);
    ...
    count = MIN((size_t)(bufend - rb->head), count);
    n = fread(rb->head, 1, count, fd);
    if (n > 0) {
...
ringbuf_memcpy_from(void *dst, ringbuf_t src, size_t count)
{
    ...
    while (nwritten != count) {
        ...
        n = (size_t)MIN((size_t)(bufend - src->tail), count - nwritten);
        if(u8dst!=0)memcpy(u8dst + nwritten, src->tail, n);
        src->tail += n;
        nwritten += n;
...
ssize_t
ringbuf_write(FILE* fd, ringbuf_t rb, size_t count)
{
    size_t bytes_used = ringbuf_bytes_used(rb);
	const uint8_t * bufend = 0;
	ssize_t n = 0;
    ...
    bufend = ringbuf_end(rb);
    assert(bufend > rb->head);
    count = MIN((size_t)(bufend - rb->tail), count);
    n = fwrite(rb->tail, 1, count, fd);
    if (n > 0) {
...
ringbuf_copy(ringbuf_t dst, ringbuf_t src, size_t count)
{
    ...
    const uint8_t *src_bufend = 0;
    const uint8_t *dst_bufend = 0;
    size_t ncopied = 0;
	size_t nsrc = 0;
	size_t n = 0;
    ...
    src_bufend = ringbuf_end(src);
    dst_bufend = ringbuf_end(dst);
    ncopied = 0;
    while (ncopied != count) {
        assert(src_bufend > src->tail);
        nsrc = (size_t)MIN((size_t)(src_bufend - src->tail), count - ncopied);
        assert(dst_bufend > dst->head);
        n = (size_t)MIN((size_t)(dst_bufend - dst->head), nsrc);
